...
def task_list_view(request):
    if request.user.is_superuser:
        ...
        tasks = Job.objects.filter(Q(assigned_to=request.user) | Q(created_by=request.user)) 
...
def add_note_view(request, task_id):
    """Add a new note or edit the last note if created by the current user."""
    task = get_object_or_404(Job, id=task_id)
...
def task_history_view(request, task_id):
    """View the history log of changes made to the task."""
    task = get_object_or_404(Job, id=task_id)
